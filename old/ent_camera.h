...
class ent_camera: public ent_3d_2a
{
	public:
		...
		std::string get_member(std::string member_name) override
		{
			if (member_name == "fov") { return std::to_string(fov); }
			else if (member_name == "near") { return std::to_string(near_clip); }
			else if (member_name == "far") { return std::to_string(far_clip); }
			//else if (member_name == "ubo") { return std::to_string(ubo.bufferID); }
			//else if (member_name == "view") { return std::to_string(ubo.bufferID); }



		}
		static const char* get_name() { return "ent_camera"; }
		...
		virtual void tick() override; // implemented later
