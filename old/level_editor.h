...
public:
    ...
    typeless_array& brushes = globals.scene.from_typename<ent_brush>();
    //std::vector<ent_brush> ;
    aligned_ptr<ent_brush> brush_iterable;

    level_editor_app(int argc, char** argv): Application()//,console(custom_state)
    {
        console.open = &input_to_imgui;
        brush_iterable = brushes.get_start<ent_brush>();
    }
    ...
    void gl_tick() override {
        ...
        // Draw the brushes
        for (size_t i = 0; i < brushes.get_size(); i++)
        {
            brush_iterable[i].tick();
        }
    ...
    void bootstrap() override {
        ...
        brushes.emplace_back<ent_brush>(vec3(0.0, 0.0, 0.0), vec3(1.0, 1.0, 1.0));
        brush_iterable[0].prepare_for_draw();
