...
public:
    ...
    void add_system(Args&&... args) {
        ...
        //systems.back()->init(); // Call init on the newly added system
    }
...
protected:
    friend class Client;
    friend class Server;
    inline void init_system(int index, Scene& scene) const {
        systems[index]->init(scene);
    }

    inline void tick_system(int index, Scene& scene) const {
        systems[index]->tick(scene);
    }

    inline void destroy_system(int index, Scene& scene) const {
