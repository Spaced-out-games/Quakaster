...
struct QMovement : Controller, QKComponent {
    ...
    struct system : ISystem {
        ...
        void tick(QKScene& scene) override {
            ...
            for (auto entity : view) {
                ...
                // Update downward velocity due to gravity
                ms.set_downward_velocity(ms.get_downward_velocity() - (ms.gravity() * dt));
                ...
                if (distance_from_plane < 2.0f) {  // 2.0f accounts for player height
                    ...
                    tf.position -= ground_plane.normal * (distance_from_plane - 2.0f); // Correct position to rest on the plane
                }
                else {
                    ms.set_in_air(true);
                }
    ...
    QMovement(QKEntity& entity) : Controller(), entity(entity) {
...
void QMovement::handle_movement(SDL_Keycode code, MoveState& ms) {
...
inline void QMovement::apply_movement(MoveState& ms, QKEntity& entity) {
    Transform& transform = entity.get<Camera>().owner_transform; // easier to test immediately
    if (ms.moving()) {
        if (ms.in_air()) {
...
inline void QMovement::apply_ground_accel(MoveState& ms) {
...
void QMovement::apply_air_accel(MoveState& ms) {
    // Ensure wish_dir is not a zero vector
    if (glm::length(ms.wish_dir()) > 0.0f) {
        ms.set_wish_dir(glm::normalize(ms.wish_dir()));
    }
