...
struct res_mesh{
        ...
        : index_count(index_count)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, vertex_count * vertex_size, vertices, DrawingType);
    ...
    void addVec2(GLuint index, std::size_t offset) {
        glVertexAttribPointer(index, 2, GL_FLOAT, GL_FALSE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addVec3(GLuint index, std::size_t offset) {
        glVertexAttribPointer(index, 3, GL_FLOAT, GL_FALSE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addVec4(GLuint index, std::size_t offset) {
        glVertexAttribPointer(index, 4, GL_FLOAT, GL_FALSE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    // New methods for integer types
    void addInt8(GLuint index, std::size_t offset, bool normalized = false) {
        glVertexAttribIPointer(index, 1, GL_BYTE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addUInt8(GLuint index, std::size_t offset, bool normalized = false) {
        glVertexAttribIPointer(index, 1, GL_UNSIGNED_BYTE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addInt16(GLuint index, std::size_t offset) {
        glVertexAttribIPointer(index, 1, GL_SHORT, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addUInt16(GLuint index, std::size_t offset) {
        glVertexAttribIPointer(index, 1, GL_UNSIGNED_SHORT, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addInt32(GLuint index, std::size_t offset) {
        glVertexAttribIPointer(index, 1, GL_INT, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addUInt32(GLuint index, std::size_t offset) {
        glVertexAttribIPointer(index, 1, GL_UNSIGNED_INT, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }
    ...
    void addUInt64(GLuint index, std::size_t offset) {
        ...
        glVertexAttribIPointer(index, 2, GL_UNSIGNED_INT, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    // New methods for floating-point types (float and double)
    void addFloat(GLuint index, std::size_t offset) {
        glVertexAttribPointer(index, 1, GL_FLOAT, GL_FALSE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }
    ...
    void addDouble(GLuint index, std::size_t offset) {
        glVertexAttribLPointer(index, 1, GL_DOUBLE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addVec2Double(GLuint index, std::size_t offset) {
        glVertexAttribLPointer(index, 2, GL_DOUBLE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addVec3Double(GLuint index, std::size_t offset) {
        glVertexAttribLPointer(index, 3, GL_DOUBLE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }

    void addVec4Double(GLuint index, std::size_t offset) {
        glVertexAttribLPointer(index, 4, GL_DOUBLE, vertex_size, (GLvoid*)offset);
        glEnableVertexAttribArray(index);
    }
