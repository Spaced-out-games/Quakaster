...
void add_event_listeners() \
    { \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, KeyPressEvent, on_KeyPress); \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, KeyHoldEvent, on_KeyHeld); \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, KeyReleaseEvent, on_KeyRelease); \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, MouseClickEvent, on_MouseClick); \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, MouseHoldEvent, on_MouseClickHold); \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, MouseReleaseEvent, on_MouseClickRelease); \
        ADD_EVENT_LISTENER(client.cl_scene.dispatcher, MouseButtonClickEvent, on_MouseButtonClick); \
    } \
...
public:
    ...
    // Constructor: Initializes the Controller with the given client instance
    explicit Controller(Client& client)
        : client(client), lastMouseX(0), lastMouseY(0) {}
    ...
    void dispatchIOEvents() {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            case SDL_MOUSEBUTTONUP: {
                ...
                if (event.button.clicks > 1) {
                    MouseButtonClickEvent mouseButtonClickEvent{
                        ...
                        event.button.y,
                        event.button.clicks };
                    client.cl_scene.dispatcher.trigger(mouseButtonClickEvent);
                }
            } break;
            ...
            case SDL_MOUSEMOTION: {
                ...
                MouseMoveEvent mouseMoveEvent{ event.motion.x, event.motion.y, deltaX, deltaY };
                client.cl_scene.dispatcher.trigger(mouseMoveEvent);
            ...
            case SDL_QUIT:
                // Handle quit event (e.g., exit the client loop)
                exit(0);
                break;
...
public:
    ...
    Entity& target;
    basic_controller(Client& client, Entity& pawn) : Controller(client), target(pawn) { add_event_listeners(); }
